...
Threadline is an AI-powered presentation builder. This repository contains both the marketing site and the signed‑in dashboard, along with the Supabase/HubSpot integration code. The UI is built with React, Vite and Tailwind CSS using the shadcn/ui component library. The project targets **Node.js 18** or newer and this is enforced via the `engines` field in `package.json`, so ensure that version is installed locally.
...
Ensure **Node.js 18** or newer is installed to match the CI environment. The requirement is enforced via the `engines` field.
