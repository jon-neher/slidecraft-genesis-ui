...
-- Handle existing data and type conversion properly

-- Drop existing foreign key constraints that might conflict
ALTER TABLE public.profiles DROP CONSTRAINT IF EXISTS profiles_id_fkey;

-- Drop all policies that depend on profiles.id column
...
-- Clear existing data in profiles since we need to change the type
TRUNCATE TABLE public.profiles;

-- Now we can safely alter the column type
ALTER TABLE public.profiles ALTER COLUMN id TYPE TEXT;

-- Insert users that exist in HubSpot tables into profiles
INSERT INTO public.profiles (id, created_at, updated_at)
SELECT DISTINCT user_id, now(), now()
FROM public.hubspot_oauth_states 
ON CONFLICT (id) DO NOTHING;

INSERT INTO public.profiles (id, created_at, updated_at)
SELECT DISTINCT portal_id, now(), now()
FROM public.hubspot_tokens 
ON CONFLICT (id) DO NOTHING;
